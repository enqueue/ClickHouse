...
void HashedArrayDictionary<dictionary_key_type, sharded>::loadData()
{
    if (!source_ptr->hasUpdateField())
    {
        ...
        {
            if constexpr (std::is_same_v<T, QueryPipeline>)
            {
                loadDataImpl(arg, nullptr, parallel_loader.get());
            }
            else if constexpr (std::is_same_v<T, BlockIO>)
            {
                loadDataImpl(arg.pipeline, &arg, parallel_loader.get());
            }
        }, source_ptr->loadAllWithOptionalBlockIO());
    }
    else
    {
        updateData();
...
template <DictionaryKeyType dictionary_key_type, bool sharded>
void HashedArrayDictionary<dictionary_key_type, sharded>::loadDataImpl(
    QueryPipeline & pipeline,
    BlockIO * io,
    DictionaryParallelLoaderType * parallel_loader)
try
{
    DictionaryPipelineExecutor executor(pipeline, configuration.use_async_executor);
    pipeline.setConcurrencyControl(false);
